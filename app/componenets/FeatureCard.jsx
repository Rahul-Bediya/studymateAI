...
export default function FeatureCard({ icon, title, desc, features, link }) {
  ...
  const { user } = useAuth(); // assuming you store user info in context

  const handleClick = () => {
    if (!user) {
      router.push('/auth'); // redirect to login if not logged in
    } else {
      router.push(link); // go to feature page if logged in
    }
  };
  ...
  return (
    <motion.div
      whileHover={{
        scale: 1.03,
        boxShadow:
          '0px 12px 24px rgba(137, 207, 240, 0.4), 0px 8px 16px rgba(255, 192, 203, 0.2)',
      }}
      transition={{ type: 'spring', stiffness: 300, damping: 20 }}
      className="bg-white rounded-2xl border border-gray-200 p-6 text-left shadow-sm transition-all duration-300"
    >
